...
def get_config_obj(
    ...
    conf_filename = conf_name if "." in conf_name else "".join([conf_name, ".cfg"])
                ...
                if "attr_dict" in v:
                    for key in v["attr_dict"]:
                        nx.set_edge_attributes(
                            graph, {(i, j, k): {key: v["attr_dict"][key]}}
                        )
        ...
        for k, lpu in lpus.items():
            ...
            for i, j, ko, v in graph.edges(keys=True, data=True):
                if "attr_dict" in v:
                    for key in v["attr_dict"]:
                        nx.set_edge_attributes(
                            graph, {(i, j, ko): {key: v["attr_dict"][key]}}
                        )
            ...
            if k == "retina":
                ...
                input_processor = StepInputProcessor(
                    "I",
                    [
                        node[0]
                        for node in graph.nodes(data=True)
            ...
            output_processor = FileOutputProcessor(
                [("V", lpu["output_uid_list"])], lpu["output_file"], sample_interval=10
            )
        ...
        for k, pattern in patterns.items():
            ...
            if l1 in lpus and l2 in lpus:
                ...
                pat, key_order = Pattern.from_graph(
                    nx.DiGraph(pattern["graph"]), return_key_order=True
                )
                print(l1, l2)
                print(key_order)
        ...
        if "retina" in lpus:
            ...
            for i, item in enumerate(retina_input_uids):
                inputs["data"][item] = np.hstack(
                    (
                        np.arange(int((steps - ignored_steps) / 10)).reshape((-1, 1))
                        * dt
                        * 10,
                        input_array[ignored_steps::10, i : i + 1],
                    )
                ).tolist()
