...
class ffbolabComponent:
    def __init__(self, ssl = True, debug = True, authentication = True, user = u"ffbo", secret = u"", url = u'wss://neuronlp.fruitflybrain.org:7777/ws', realm = u'realm1', ca_cert_file = 'isrgrootx1.pem', intermediate_cert_file = 'letsencryptauthorityx3.pem', FFBOLabcomm = None):
        ...
        def updateFileList():
            self.files = [f for f in listdir(_FFBOLabDataPath) if isfile(join(_FFBOLabDataPath, f))]
        ...
        def loadResults(details = None):
            ...
            for i in range(no_keys):
                dicttosend = dict((k,json_data[k]) for k in list(json_data.keys())[i*5:(i+1)*5] if k in list(json_data.keys()))
                #try:
...
def mainThreadExecute(Component):
    ...
    if len(Component.executionSettings)>0:
        ...
        if os.path.isfile(os.path.join(_FFBOLabDataPath, name + '.json')):
            ...
            C_in_ex = nb.InIStep(0, 40., 0.20, 0.40)
