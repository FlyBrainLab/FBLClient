...
def diagram_generator(X, log, prog = 'dot', method = None, splines = 'line', rename = False, styles = None, **kwargs):
    ...
    if styles is None:
        if method == 'sugiyama':
            size_mult = str(int(np.round(128. * np.sqrt(nx.number_of_edges(G)))))
            styles = {
                'graph': {
                    'label': name,
                    'fontsize': '16',
